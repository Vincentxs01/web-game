...
export class GameCoordinator {
  ...
  getMenuStartButtonRect() {
    const titleCenterY = SCREEN_HEIGHT / 2;
    const btnW = 220;
    const btnH = 56;
    return {
  ...
  bindEvents() {
    ...
    const onFirstInteraction = () => {
      void soundManager.ensureStarted();
    };
    this.canvas.addEventListener('pointerdown', (e) => {
      ...
      this.pointer.x = p.x;
      this.pointer.y = p.y;
      void this.onPointerDown(p.x, p.y, e.button);
      if (this.state === 'GAMEPLAY') this.canvas.setPointerCapture(e.pointerId);
    });
    document.getElementById('wrap')?.addEventListener('pointerdown', onFirstInteraction, {
      once: false,
    });
  ...
  async onPointerDown(x, y, button) {
    if (this.state === 'MENU' && button === 0) {
      const btn = this.getMenuStartButtonRect();
  ...
  measureTitleWidth(ctx, title, letterGap = 4, spaceWidth = 20) {
    let total = 0;
    for (let i = 0; i < title.length; i++) {
      const ch = title[i];
      if (ch === ' ') total += spaceWidth;
      else {
        total += ctx.measureText(ch).width;
        if (i < title.length - 1 && title[i + 1] !== ' ') total += letterGap;
      }
    }
    return total;
  }

  drawMenuTitle(ctx, title, centerX, centerY, timeMs = 0) {
    const letterGap = 4;
    const spaceWidth = 20;
    const totalWidth = this.measureTitleWidth(ctx, title, letterGap, spaceWidth);
    ...
    for (let i = 0; i < title.length; i++) {
      ...
      const wave = Math.sin(timeMs * 0.005 + i * 0.6) * 7;
  ...
  renderMenu() {
    ...
    const titleCenterX = SCREEN_WIDTH / 2;
    const titleCenterY = SCREEN_HEIGHT / 2;
    this.drawMenuTitle(ctx, 'ANGRY BIRDS', titleCenterX, titleCenterY, t);
    ...
    const btnW = btn.w;
    const btnH = btn.h;
    const hover = this.hitRect(this.pointer.x, this.pointer.y, btn);
